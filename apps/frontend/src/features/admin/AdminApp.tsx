...
import {
  ...
  loginAdmin,
  logoutAdmin,
  publishAdminQrContent,
  saveAdminQrContent,
  saveAdminQrOverride,
  setAdminQrOverrideStatus,
} from "./adminApi";
    ...
    <div className="adminRoot adminShell">
      ...
      <aside className={`adminSidebar ${menuOpen ? "is-open" : ""}`} aria-label="Điều hướng quản trị">
        ...
        <nav className="adminNav">
          ...
          {navItems.filter((item) => !session.permissions || session.permissions.includes(item.permission)).map(({ href, label, icon: Icon }) => {
  ...
  return (
    <>
      ...
      <section className="adminKpiGrid" aria-label="Chỉ số tổng quan">
        <KpiCard label="Tổng lượt quét QR" value={(dashboard?.totalScans ?? 0).toLocaleString("vi-VN")} delta="Theo khoảng thời gian đã chọn" icon={<QrCode size={20} />} />
        <KpiCard label="Yêu cầu đang xử lý" value={String(dashboard?.openLeads ?? 0)} delta={`${dashboard?.submissionsByStatus.new ?? 0} yêu cầu mới`} icon={<UserCheck size={20} />} tone="gold" />
        <KpiCard label="QR đang hoạt động" value={String(dashboard?.activeQrRecords ?? 0)} delta="Dữ liệu trực tiếp từ backend" icon={<Activity size={20} />} tone="blue" />
        <KpiCard label="Sản phẩm công khai" value={String(dashboard?.activeProducts ?? 0)} delta="Danh mục đang hoạt động" icon={<Package size={20} />} tone="pink" />
      </section>
...
function ProductsPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Danh mục" title="Sản phẩm" description="Quản lý nội dung hiển thị, trạng thái phát hành và các biến thể của Senova." action={canWrite ? <Link className="adminButton adminButton--primary" href="/admin/products/new"><Plus size={16} />Thêm sản phẩm</Link> : undefined} />
      <section className="adminCard">
...
function ProductFormPage({ id }: { id?: string }) {
  const { products, saveProduct, setProductStatus, notify, loadProducts, isLoading } = useAdminStore();
  const { navigate } = useRouter();
  ...
  if (id && !existing) return isLoading || products.length === 0
    ? <section className="adminCard"><EmptyState title="Đang tải sản phẩm" description="Vui lòng chờ dữ liệu từ backend." /></section>
    : <NotFoundPage />;
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      // The store keeps the form intact and shows the typed API error.
    } finally {
      setSaving(false);
    }
  };
...
function QrPage() {
  const { qrRecords, products, loadQr, loadProducts } = useAdminStore();
  const [query, setQuery] = useState("");
  const [status, setStatus] = useState("all");
  const [page, setPage] = useState(1);
...
function QrFormPage({ id }: { id?: string }) {
  ...
  const { navigate } = useRouter();
  const existing = id ? qrRecords.find((record) => record.code === decodeURIComponent(id)) : undefined;
  const defaultProduct = products[0]?.slug ?? "";
  const [form, setForm] = useState<QrRecord>(existing ?? { id: "", code: "", productSlug: defaultProduct, batchCode: "", contentVersion: DEFAULT_QR_CONTENT_VERSION, destination: `/experience/${defaultProduct}`, campaign: "", locale: "vi", status: "paused", scans: 0, createdAt: new Date().toISOString() });
  ...
  const submit = async (event: FormEvent) => {
    ...
    setSaving(true);
    const record = { ...form, id: form.code, status: existing?.status ?? "paused", destination: `/experience/${form.productSlug}` };
    try {
      const saved = await saveQrRecord(record);
      notify(existing ? "Đã lưu thông tin mã QR." : "Đã tạo mã QR mới.");
      navigate(`/admin/qr/${encodeURIComponent(saved.code)}`);
    } catch {
      // The store keeps the form intact and shows the typed API error.
    } finally {
      setSaving(false);
  ...
  const changeStatus = async () => { if (!confirm || !existing) return; await setQrStatus(existing.code, confirm); setForm((current) => ({ ...current, status: confirm })); notify(`Đã chuyển ${existing.code} sang trạng thái mới.`); setConfirm(null); };
...
function SubmissionsPage() {
  const { submissions, loadSubmissions } = useAdminStore();
...
function SubmissionDetailPage({ id }: { id: string }) {
  ...
  if (!submission) return isLoading
    ...
    : <NotFoundPage />;
  const save = async () => { setSaving(true); try { await updateSubmission(submission.id, status, assignee); notify("Đã cập nhật yêu cầu khách hàng."); } catch { /* Store shows the typed error. */ } finally { setSaving(false); } };
...
function QrContentPage() {
  ...
  const selectContent = (content: AdminQrContent) => { setForm(content); setStepsJson(JSON.stringify(content.guidance.steps, null, 2)); setCloneVersion(""); };
  const updateNested = (section: "story" | "culture" | "guidance", key: string, value: unknown) => {
    setForm((current) => ({ ...current, [section]: { ...current[section], [key]: value } }));
  };
  const saveContent = async (event: FormEvent) => {
    event.preventDefault();
    if (!form.productSlug || !form.version.trim() || !form.title.trim()) { notify("Vui lòng nhập sản phẩm, version và tiêu đề.", "error"); return; }
    let steps: AdminQrContent["guidance"]["steps"];
    try { steps = JSON.parse(stepsJson) as AdminQrContent["guidance"]["steps"]; }
    catch { notify("Các bước hướng dẫn phải là JSON hợp lệ.", "error"); return; }
    const saved = await saveAdminQrContent({ ...form, guidance: { ...form.guidance, steps }, version: form.version.trim(), status: "draft" });
    setForm(saved); await loadQrContentData(); notify("Đã lưu bản nháp nội dung QR.");
  };
  const clone = async () => {
    if (!cloneVersion.trim()) { notify("Vui lòng nhập mã version mới.", "error"); return; }
    let steps: AdminQrContent["guidance"]["steps"];
    try { steps = JSON.parse(stepsJson) as AdminQrContent["guidance"]["steps"]; }
    catch { notify("Các bước hướng dẫn phải là JSON hợp lệ.", "error"); return; }
    const saved = await saveAdminQrContent({ ...form, guidance: { ...form.guidance, steps }, version: cloneVersion.trim(), status: "draft", updatedAt: undefined });
    setForm(saved); setCloneVersion(""); await loadQrContentData(); notify("Đã tạo version QR mới.");
  };
  const publish = async () => {
    const published = await publishAdminQrContent(form.productSlug, form.version, form.locale);
    setForm(published); setPublishing(false); await loadQrContentData(); notify("Đã publish nội dung QR.");
  };
  const selectOverride = (item: AdminQrOverride) => {
    setOverride(item); setGuidanceJson(item.guidanceOverride ? JSON.stringify(item.guidanceOverride, null, 2) : "");
  };
  const saveOverride = async (event: FormEvent) => {
    event.preventDefault();
    let guidanceOverride: AdminQrOverride["guidanceOverride"];
    try { guidanceOverride = guidanceJson.trim() ? JSON.parse(guidanceJson) as AdminQrOverride["guidanceOverride"] : undefined; }
    catch { notify("Guidance override phải là JSON hợp lệ.", "error"); return; }
    const saved = await saveAdminQrOverride({ ...override, batchCode: override.batchCode.trim().toUpperCase(), guidanceOverride });
    setOverride(saved); await loadQrContentData(); notify("Đã lưu batch override.");
  };

  return <>
    <PageHeader eyebrow="Trải nghiệm số" title="Nội dung QR" description="Quản lý draft, version đã publish và hướng dẫn riêng theo lô." action={canManage ? <button className="adminButton adminButton--primary" type="button" onClick={() => { const fresh = emptyQrContent(products[0]?.slug); setForm(fresh); setStepsJson(JSON.stringify(fresh.guidance.steps, null, 2)); }}><Plus size={16} />Tạo nội dung</button> : undefined} />
    <div className="adminGrid">
      <section className="adminCard"><div className="adminCard__header"><div><h2>Phiên bản nội dung</h2><p>Published content là bất biến</p></div></div><div className="adminCard__body">
        {qrContents.length ? <div className="adminTableWrap"><table className="adminTable"><thead><tr><th>Sản phẩm</th><th>Version</th><th>Locale</th><th>Trạng thái</th></tr></thead><tbody>{qrContents.map((item) => <tr key={`${item.productSlug}:${item.version}:${item.locale}`} onClick={() => selectContent(item)}><td><button type="button" className="adminTable__link">{item.productSlug}</button></td><td>{item.version}</td><td>{item.locale.toUpperCase()}</td><td><StatusBadge status={item.status} /></td></tr>)}</tbody></table></div> : <EmptyState title="Chưa có nội dung QR" description="Tạo draft đầu tiên để bắt đầu." />}
      </div></section>
      <form className="adminCard" onSubmit={saveContent}><div className="adminCard__header"><div><h2>{form.title || "Nội dung mới"}</h2><p>{readOnly ? "Chỉ đọc — clone để chỉnh sửa" : "Bản nháp có thể chỉnh sửa"}</p></div>{form.status ? <StatusBadge status={form.status} /> : null}</div><div className="adminCard__body"><div className="adminFormGrid">
        <div className="adminField"><label>Sản phẩm</label><select disabled={readOnly} value={form.productSlug} onChange={(event) => setForm(emptyQrContent(event.target.value, form.version, form.locale))}>{products.map((item) => <option key={item.id} value={item.slug}>{item.name}</option>)}</select></div>
        <div className="adminField"><label>Version</label><input disabled={readOnly} value={form.version} onChange={(event) => setForm((value) => ({ ...value, version: event.target.value }))} /></div>
        <div className="adminField"><label>Ngôn ngữ</label><select disabled={readOnly} value={form.locale} onChange={(event) => setForm((value) => ({ ...value, locale: event.target.value as "vi" | "en" }))}><option value="vi">Tiếng Việt</option><option value="en">English</option></select></div>
        <div className="adminField"><label>Content viewed</label><input disabled={readOnly} value={form.contentViewed} onChange={(event) => setForm((value) => ({ ...value, contentViewed: event.target.value }))} /></div>
        <div className="adminField adminField--full"><label>Eyebrow</label><input disabled={readOnly} value={form.eyebrow} onChange={(event) => setForm((value) => ({ ...value, eyebrow: event.target.value }))} /></div>
        <div className="adminField adminField--full"><label>Tiêu đề</label><input disabled={readOnly} value={form.title} onChange={(event) => setForm((value) => ({ ...value, title: event.target.value }))} /></div>
        <div className="adminField adminField--full"><label>Mở đầu</label><textarea disabled={readOnly} value={form.lede} onChange={(event) => setForm((value) => ({ ...value, lede: event.target.value }))} /></div>
        <div className="adminField adminField--full"><label>Tiêu đề câu chuyện</label><input disabled={readOnly} value={form.story.title} onChange={(event) => updateNested("story", "title", event.target.value)} /></div>
        <div className="adminField adminField--full"><label>Các đoạn câu chuyện — mỗi dòng một đoạn</label><textarea disabled={readOnly} value={form.story.paragraphs.join("\n")} onChange={(event) => updateNested("story", "paragraphs", event.target.value.split("\n"))} /></div>
        <div className="adminField adminField--full"><label>Tiêu đề văn hóa</label><input disabled={readOnly} value={form.culture.title} onChange={(event) => updateNested("culture", "title", event.target.value)} /></div>
        <div className="adminField adminField--full"><label>Nội dung văn hóa — mỗi dòng một đoạn</label><textarea disabled={readOnly} value={form.culture.paragraphs.join("\n")} onChange={(event) => updateNested("culture", "paragraphs", event.target.value.split("\n"))} /></div>
        <div className="adminField adminField--full"><label>Tiêu đề hướng dẫn</label><input disabled={readOnly} value={form.guidance.title} onChange={(event) => updateNested("guidance", "title", event.target.value)} /></div>
        <div className="adminField adminField--full"><label>Mở đầu hướng dẫn</label><textarea disabled={readOnly} value={form.guidance.intro} onChange={(event) => updateNested("guidance", "intro", event.target.value)} /></div>
        <div className="adminField adminField--full"><label>Các bước hướng dẫn (JSON)</label><textarea disabled={readOnly} value={stepsJson} onChange={(event) => setStepsJson(event.target.value)} /></div>
        <div className="adminField adminField--full"><label>Câu hỏi suy ngẫm</label><textarea disabled={readOnly} value={form.reflectionPrompt} onChange={(event) => setForm((value) => ({ ...value, reflectionPrompt: event.target.value }))} /></div>
      </div><div className="adminFormActions">{readOnly ? <><input aria-label="Version mới" placeholder="v2 hoặc campaign-2026" value={cloneVersion} onChange={(event) => setCloneVersion(event.target.value)} /><button type="button" className="adminButton adminButton--secondary" onClick={clone}>Clone thành draft</button></> : <><button className="adminButton adminButton--primary" type="submit"><Save size={15} />Lưu draft</button><button className="adminButton adminButton--secondary" type="button" onClick={() => setPublishing(true)}>Publish</button></>}</div></div></form>
    </div>
    <section className="adminCard"><div className="adminCard__header"><div><h2>Batch override</h2><p>Ghi đè hướng dẫn hoặc thông báo cho đúng lô</p></div></div><div className="adminCard__body"><div className="adminGrid"><div>{qrOverrides.map((item) => <button type="button" className="adminButton adminButton--ghost" key={`${item.batchCode}:${item.productSlug}:${item.contentVersion}`} onClick={() => selectOverride(item)}>{item.batchCode} · {item.contentVersion} · {item.status}</button>)}</div><form onSubmit={saveOverride}><div className="adminFormGrid"><div className="adminField"><label>Mã lô</label><input value={override.batchCode} onChange={(event) => setOverride((value) => ({ ...value, batchCode: event.target.value }))} /></div><div className="adminField"><label>Sản phẩm</label><select value={override.productSlug} onChange={(event) => setOverride((value) => ({ ...value, productSlug: event.target.value }))}><option value="">Chọn sản phẩm</option>{products.map((item) => <option key={item.id} value={item.slug}>{item.name}</option>)}</select></div><div className="adminField adminField--full"><label>Published version</label><select value={override.contentVersion} onChange={(event) => setOverride((value) => ({ ...value, contentVersion: event.target.value }))}><option value="">Chọn version</option>{qrContents.filter((item) => item.status === "published" && (!override.productSlug || item.productSlug === override.productSlug)).map((item) => <option key={`${item.productSlug}:${item.version}:${item.locale}`} value={item.version}>{item.version} · {item.locale}</option>)}</select></div><div className="adminField adminField--full"><label>Thông báo</label><textarea value={override.notice ?? ""} onChange={(event) => setOverride((value) => ({ ...value, notice: event.target.value }))} /></div><div className="adminField adminField--full"><label>Guidance override (JSON, để trống nếu không ghi đè)</label><textarea value={guidanceJson} onChange={(event) => setGuidanceJson(event.target.value)} /></div></div><div className="adminFormActions"><button className="adminButton adminButton--primary" type="submit">Lưu override</button>{override.batchCode ? <button className="adminButton adminButton--ghost" type="button" onClick={async () => { const saved = await setAdminQrOverrideStatus(override, override.status === "active" ? "disabled" : "active"); setOverride(saved); await loadQrContentData(); }}>{override.status === "active" ? "Disable" : "Activate"}</button> : null}</div></form></div></div></section>
    <ConfirmDialog open={publishing} title="Publish nội dung QR?" description="Version đã publish sẽ bất biến; muốn chỉnh sửa sau đó phải clone sang version mới." confirmLabel="Publish" onCancel={() => setPublishing(false)} onConfirm={publish} />
  </>;
}

function AnalyticsPage() {
  const { analytics, products, dashboard, loadDashboard, loadProducts } = useAdminStore();
  const [period, setPeriod] = useState("7"); const [product, setProduct] = useState("all"); const [source, setSource] = useState("all");
  const totalScans = dashboard?.totalScans ?? 0;
  const selectedSubmissions = dashboard?.totalSubmissions ?? 0;
  const conversion = dashboard?.conversionRate ?? 0;
  const breakdown = products.map((item) => ({ label: item.name.replace("Senova ", ""), value: dashboard?.scansByProduct.find((entry) => entry.productSlug === item.slug)?.scans ?? 0 }));
...
export default function AdminApp() {
  ...
  useEffect(() => {
    const unauthorized = () => { setSession(null); navigate("/admin/login"); };
    window.addEventListener("senova:admin-unauthorized", unauthorized);
    return () => window.removeEventListener("senova:admin-unauthorized", unauthorized);
  }, [navigate]);
